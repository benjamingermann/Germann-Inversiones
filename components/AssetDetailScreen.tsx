    ...
    <div className="flex flex-col min-h-screen bg-background-dark pb-32 animate-fade-in">
      <header className="sticky top-0 z-50 flex items-center bg-background-dark/95 backdrop-blur-xl p-4 pb-2 justify-between border-b border-white/5">
        ...
        <div className="flex flex-col items-center">
          ...
          <span className="text-[10px] font-bold text-primary uppercase tracking-tighter">Cotización en Vivo</span>
        </div>
      ...
      <main className="px-6 max-w-4xl mx-auto w-full">
        ...
        <section className="mt-4 mb-8 relative">
          <div className="absolute -inset-1 bg-primary/20 rounded-[2rem] blur-xl opacity-20"></div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          ...
          <div className="bg-surface-dark/40 p-5 rounded-2xl border border-white/5">
            <p className="text-[10px] font-bold text-gray-500 uppercase mb-1">Valor Actual</p>
            <p className="text-xl font-bold text-primary">u$s {(asset.quantity * asset.price).toLocaleString('es-AR', { maximumFractionDigits: 0 })}</p>
          </div>
          <div className="bg-surface-dark/40 p-5 rounded-2xl border border-white/5">
            <p className="text-[10px] font-bold text-gray-500 uppercase mb-1">Apertura</p>
            <p className="text-xl font-bold text-white">{(asset.price * 0.99).toFixed(2)}</p>
          </div>
          <div className="bg-surface-dark/40 p-5 rounded-2xl border border-white/5">
        ...
        <h3 className="text-white text-lg font-bold mb-4 px-2">Noticias Relacionadas</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {NEWS_ITEMS.map(news => (
            <div key={news.id} className="flex gap-4 p-4 bg-surface-dark/20 rounded-2xl border border-white/5 hover:bg-surface-dark/40 transition-all cursor-pointer group">
              ...
              <img src={news.imageUrl} className="size-14 rounded-xl object-cover opacity-60 group-hover:opacity-100 transition-opacity" alt="" />
            </div>
          ))}
        </div>
      </main>
